# OpenZL Assembler - Phase 1

A simple assembler for the OpenZL VM bytecode format.
...
```bash
python3 assembler.py -c "push.zero halt"
# Output: 01 00 01 00 05 00 01 00
```

### Save command-line output to file
```bash
python3 assembler.py -c "push.zero halt" output.bin
```
...
- `test_*.asm` - Example assembly files
- `*.bin` - Generated bytecode files
