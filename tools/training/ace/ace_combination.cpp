        ...
        size_t numCandidates)
{
    ...
    for (const auto& candidate : candidates) {
        fitness.emplace_back(candidate.computeFitness());
        ...
        size_t maxNumCandidates)
{
    ...
    auto filtered = filterParetoFrontier(std::move(newFrontier), threadPool);
    auto pruned   = pruneCandidates(std::move(filtered), maxNumCandidates);
    return pruned;
}
        ...
        const TrainParams& trainParams)
{
    ...
    auto currentFrontier = combineCandidates(candidates, trainParams);
    /* TODO: Alternative pruning for surfacing candidates to users*/
    auto finalFrontier = pruneCandidates(
            std::move(currentFrontier), kNumFinalParetoCandidates);
    std::sort(finalFrontier.begin(), finalFrontier.end());
    std::vector<std::shared_ptr<const std::string_view>> paretoOptimalResults;
    paretoOptimalResults.reserve(finalFrontier.size());
    for (auto& candidate : finalFrontier) {
