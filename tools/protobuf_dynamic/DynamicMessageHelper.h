...
public:
     ...
     * 
     * @param type_name Fully qualified message type (e.g., "mypackage.MyMessage")
     * @return Unique pointer to new message, or nullptr if type not found
     */
    std::unique_ptr<google::protobuf::Message> newMessage(const std::string& type_name) const;
     ...
     */
    const google::protobuf::Descriptor* getDescriptor(const std::string& type_name) const;
    
    /**
     * Get the underlying descriptor pool.
     */
    const google::protobuf::DescriptorPool* getPool() const { return pool_; }
