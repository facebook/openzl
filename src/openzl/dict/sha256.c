...
// forward decl
void SHA256(const unsigned char* data, size_t len, unsigned char* digest);
...
size_t ZL_SHA256_hash(const ZL_SHA256* key)
{
    ...
    // SHA-256 already has excellent distribution, so we can just use the
    // first sizeof(size_t) bytes directly as the hash value
    size_t result;
    memcpy(&result, key->bytes, sizeof(result));
    return result;
}
