...
describe('zstd initialisation', () => {
  it('retries after a failed load instead of caching the failure', async () => {
    ...
    // below only needs a module that is already up, which the cases above left
    // it, and a second real one would spend an initialisation the module has
    // few of. The same reason `getMaxIterations`'s test stubs its own retry.
  ...
  it('survives more benchmark runs than it takes to break the module', async () => {
    ...
    // Said directly as well as survived: this module was initialised by an
    // earlier case, and the memo is what keeps these four runs off `init()`.
    expect(mockInitZstd).not.toHaveBeenCalled();
  });
});
