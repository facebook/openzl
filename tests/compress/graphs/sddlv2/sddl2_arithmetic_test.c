...
{
    SDDL2_stack* stack = malloc(sizeof(openzl_stack));
    assert(stack != NULL);
...
static void test_div_by_zero(void)
{
    ...
    assert(SDDL2_op_div(stack) == SDDL2_STACK_OVERFLOW);  // Using OVERFLOW for div-by-zero
...
static void test_mod_by_zero(void)
{
    ...
    assert(SDDL2_stack_push(stack, SDDL2_value_i64(10)) == SDDL2_OK);
    assert(SDDL2_stack_push(stack, SDDL2_value_i64(0)) == SDDL2_OK);
    assert(SDDL2_op_mod(stack) == SDDL2_STACK_OVERFLOW);  // Using OVERFLOW for div-by-zero
