...
#if defined(__EMSCRIPTEN__)
void forwardTrainingProgress(const double& progress, const std::string& message)
{
    openzl_wasm_emit_train_progress(progress, message.c_str());
}

void forwardBenchmarkProgress(
...
openzl::tools::logger::ProgressCallback makeWasmTrainingProgressCallback()
{
#if defined(__EMSCRIPTEN__)
...
openzl::tools::logger::ProgressCallback makeWasmBenchmarkProgressCallback()
{
#if defined(__EMSCRIPTEN__)
...
class ScopedProgressCallback {
   public:
    explicit ScopedProgressCallback(
            openzl::tools::logger::ProgressCallback callback)
            : active_(static_cast<bool>(callback))
    {
        if (active_) {
    ...
    {
        if (active_) {
            openzl::tools::logger::Logger::setProgressCallback({});
        }
    }
