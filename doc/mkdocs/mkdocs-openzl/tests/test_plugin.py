...
def test_workspace_packages_rejects_a_glob_pattern(tmp_path: Path):
    # Members are resolved by joining the entry to the root, so a glob would
    # resolve to a directory that does not exist instead of expanding.
    workspace = tmp_path / "web"
    workspace.mkdir()
    (workspace / "package.json").write_text(json.dumps({"workspaces": ["packages/*"]}))

    with pytest.raises(ValueError, match="Glob workspace pattern"):
        _workspace_packages(workspace)


def test_workspace_packages_rejects_a_manifest_without_workspaces(tmp_path: Path):
    workspace = tmp_path / "web"
    workspace.mkdir()
    (workspace / "package.json").write_text(json.dumps({"name": "@openzl/web"}))

    with pytest.raises(ValueError, match="has no `workspaces` array"):
        _workspace_packages(workspace)


def test_workspace_packages_rejects_duplicate_package_names(tmp_path: Path):
    # Last-wins would drop the shadowed member from the stamp and make the
    # registration check reject it as unregistered.
    workspace = tmp_path / "web"
    _create_workspace(
        workspace,
        {
            "playground": {"name": "@openzl/duplicate"},
            "visualizer": {"name": "@openzl/duplicate"},
        },
    )

    with pytest.raises(ValueError, match="Duplicate workspace package name"):
        _workspace_packages(workspace)
...
def test_web_tool_builder_ignores_generated_output_in_the_tool_itself(tmp_path: Path):
    # `dist-ssr` is generated like `dist`, so hashing it would flip the stamp on
    # every build. The tool's own directory gets the same exclusions its
    # dependency packages do.
    docs_dir = tmp_path / "docs"
    ...
    (tool_src / "dist").mkdir()
    (tool_src / "dist" / "index.html").write_text("built")
    dist_ssr = tool_src / "dist-ssr"
    dist_ssr.mkdir()
    (dist_ssr / "entry.js").write_text("first")
    ...
    (dist_ssr / "entry.js").write_text("second")
    with patch("mkdocs_openzl.plugin.check_call") as mock_call:
        builder.build()
        assert mock_call.call_count == 0

    # A real source change still rebuilds, so the exclusion is not swallowing
    # the whole tool directory.
    (tool_src / "file.txt").write_text("changed")
    with patch("mkdocs_openzl.plugin.check_call") as mock_call:
        builder.build()
        assert mock_call.call_count == 2
